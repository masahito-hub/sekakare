...
- **[👉 HANDOVER.md](HANDOVER.md)** - プロジェクト引き継ぎドキュメント（開発者向け・必読）
- [RELEASE_LOG.md](docs/RELEASE_LOG.md) - リリース履歴・バージョン管理
- [PROJECT_GOALS.md](docs/PROJECT_GOALS.md) - プロジェクト目標・ロードマップ
- [SYSTEM_SPEC.md](SYSTEM_SPEC.md) - システム仕様書
...
### 開発予定機能（Phase 2）
- 📝 ログページの編集機能（Phase 1-B）
  - 訪問日の修正
  - メニュー・写真・メモの追加
  - 並び替え機能（日付順・地域別・再訪回数順）
...
### XSS対策（重要）
- **すべての動的テキスト挿入で escapeHtml() を使用**
- utils.js の escapeHtml() 関数を使用
- innerHTML に直接データを挿入しない
- 店名・住所・メニュー・メモなどすべてエスケープ必須
...
│   │   ├── ticker.js       # ティッカー機能
│   │   ├── menu.js         # ハンバーガーメニュー
│   │   ├── utils.js        # 共通ユーティリティ（escapeHtml等）
...
├── .htaccess               # SSL・CSP設定
└── docs/
    ├── RELEASE_LOG.md      # リリース履歴
    └── PROJECT_GOALS.md    # プロジェクト目標
...
詳細は [RELEASE_LOG.md](docs/RELEASE_LOG.md) を参照してください。
