...
### v0.6.0 (2024-10-13)
- **ログページ実装**（Issue #90 → PR #92）
  - 訪問履歴の時系列表示（月ごとグループ化）
  - Empty State（ログ0件時のガイダンス）
  - 店名クリックで地図ページへズーム
  - XSS対策（escapeHtml実装）
  - トップページに「もっと見る」リンク追加
- ログページ細かい不具合修正（Issue #93 → PR #94）
  - ヘッダーテキストの改行
  - 月ヘッダーの表記修正
  - ハンバーガーメニューの z-index 調整
- menu.js のDOM要素取得タイミング修正（Issue #96 → PR #97）
  - IIFE トップレベルでの要素取得 → init() 内に移動
  - ログページでハンバーガーメニューが動作するように修正
- トップページ×ボタン重複修正（Issue #98 → PR #99）
  - `.hamburger.active` に opacity: 0 を追加
  - ハンバーガーボタンとスライドメニュー閉じるボタンの重複解消
